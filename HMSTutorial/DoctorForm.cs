...
namespace HMSTutorial
{
    ...
    {
        SqlConnection Con = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=HMSdb.mdf;Integrated Security=True;Connect Timeout=30");
        public DoctorForm()
        {
            InitializeComponent();
        }
    void populate()
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                Con.Open();
                string query = "insert into DoctorTbl values(" + DocId.Text + ",'" + DocName.Text +  "'," + DocSurname.Text + "," + DocExp.Text + ",'" + DocPass.Text + "')";
        ...
        private void DoctorGV_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            DocPass.Text = DoctorGV.SelectedRows[0].Cells[4].Value.ToString();
        }

        private void button2_Click(object sender, EventArgs e)
        {
            Con.Open();
            string query = "update DoctorTbl set DocName = '" + DocName.Text + "',DocSurname = '" + DocSurname.Text + "',DocExp ='" + DocExp.Text + "',DocPass='" + DocPass.Text + "' where DocId=" + DocId.Text + "";
            SqlCommand cmd = new SqlCommand(query, Con);
            cmd.ExecuteNonQuery();
